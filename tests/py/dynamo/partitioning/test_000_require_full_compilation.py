...
class TestRequireFullCompilation(TestCase):
    """require_full_compilation=True is documented to error rather than return a graph
    that runs partly in PyTorch.

    compile_module can return before it reaches a partitioner, when a subgraph has no
    convertible operations or fewer than min_block_size of them. The flag has to be
    honored on that path too, but only for the first reason: a graph whose every operator
    converts is fully supported however few of them there are, and all three partitioners
    deliberately disregard min_block_size in that case.
    """
    ...
    def test_small_fully_convertible_module_still_compiles(self):
        """Fewer operators than min_block_size is not partial support. All three
        partitioners compile this graph, so refusing it here would contradict them."""
        inputs = [torch.randn(8, 64, device="cuda")]
    ...
    def test_empty_graph_still_compiles(self):
