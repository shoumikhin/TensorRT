...
def test_the_python_loader_uses_the_api_that_backs_device_arenas():
    ...
    for name in ("load_model.py", "load_model_device_resident.py"):
        source = (
            _REPO_ROOT / "examples/executorch_reference_runner" / name
        ).read_text()
...
def test_the_runtime_package_imports_every_submodule_it_reaches_through():
    ...
    # os imports its public path alias itself; other dotted modules need their own import.
    missing = sorted(
        name
        for name in used
...
def test_the_delegate_follows_the_main_wheels_cuda_versions():
    """The delegate follows the validated CUDA 13 nightly rows on both Linux architectures."""
    matrix = {
        "include": [
            {
                "python_version": "3.12",
            ...
            for cuda in ("cu126", "cu130", "cu132", "cu134")
...
@pytest.mark.parametrize(
    "workflow",
    [
        "executorch-test-linux.yml",
        "release-linux-x86_64.yml",
...
def test_runtime_workflows_filter_cuda_12_without_changing_main_releases(
    ...
    """Run the real matrix steps; release rows must survive companion-only CUDA gating."""
    import shlex
    ...
    release = workflow.startswith("release-")
    job_name = "generate-release-wheel-matrix" if release else "filter-matrix"
    job = jobs[job_name]
    step = next(step for step in job["steps"] if step.get("id") == "generate")
    arch = "cuda-aarch64" if "aarch64" in workflow else "cuda"
    matrix = {
        "include": [
            {
                "python_version": "3.12",
    ...
    script = re.sub(
        r"\$\{\{ toJSON\([^\n]+\) \}\}",
        lambda _: shlex.quote(json.dumps(matrix)),
        step["run"],
    )
    script = re.sub(r"\$\{\{[^\n]+\}\}", "false", script)
    ...
    result = subprocess.run(
        ["bash", "-c", script], cwd=_REPO_ROOT, env=env, capture_output=True, text=True
    )
    assert result.returncode == 0, result.stdout + result.stderr
    outputs = dict(line.split("=", 1) for line in output.read_text().splitlines())
    if release:
        assert "executorch-matrix" not in job["outputs"]
        assert "release-executorch-runtime-wheel-artifacts" not in jobs
        assert (
            jobs["release-wheel-artifacts"]["with"]["build-executorch-runtime"] is True
        )
        assert jobs["release-wheel-artifacts"]["with"]["build-matrix"] == (
            "${{ needs.generate-release-wheel-matrix.outputs.matrix }}"
        )
        ordinary = {
...
def test_shared_repair_preserves_the_companion_payload(tmp_path, arch, floor):
    ...
    dist = tmp_path / "dist"
    dist.mkdir()
    name = "torch_tensorrt_executorch_runtime-0.1.0"
    wheel = dist / f"{name}-py3-none-linux_{arch}.whl"
    payload = b"ELF fixture: repair must not change the delegate"
    member = "torch_tensorrt_executorch_runtime/lib/libexecutorch_backend_tensorrt.so"
    with WheelFile(wheel, "w") as archive:
        archive.writestr(member, payload)
        archive.writestr(
            f"{name}.dist-info/WHEEL",
    ...
    standard = dist / f"torch_tensorrt-2.15.0-cp312-cp312-linux_{arch}.whl"
    standard.touch()
    repair = tmp_path / "test-infra/.github/scripts/repair_manylinux_2_28.sh"
    ...
    step = next(
        step
        for step in _runtime_build_steps()
        if step.get("name") == "Repair Manylinux_2_28 Wheel"
    )
    script = step["run"].replace("${{ inputs.repository }}", str(tmp_path))
    result = subprocess.run(
        ["bash", "-c", script],
        cwd=tmp_path,
        capture_output=True,
    ...
    tag = f"py3-none-manylinux_{floor}_{arch}"
    with WheelFile(dist / f"{name}-{tag}.whl") as archive:
        assert archive.read(member) == payload
        metadata = BytesParser().parsebytes(archive.read(f"{name}.dist-info/WHEEL"))
        assert metadata.get_all("Tag") == [tag]
        assert metadata["Root-Is-Purelib"] == "false"
        for member in archive.namelist():
            archive.read(member)


def _runtime_build_steps():
    import yaml

    workflow = yaml.safe_load(
        (_REPO_ROOT / ".github/workflows/build_linux.yml").read_text()
    )
...
def test_the_delegate_is_built_for_every_architecture_the_main_wheel_ships():
    ...
    shared = yaml.safe_load((workflows / "_test-linux.yml").read_text())["jobs"][
        "build"
    ]
    assert shared["uses"] == "./.github/workflows/build_linux.yml"
    assert shared["with"]["architecture"] == "${{ inputs.architecture }}"
    assert (
        shared["with"]["build-executorch-runtime"]
        == "${{ !inputs.python-only && !inputs.use-rtx }}"
    )
    for filename, arch, os_name in (
        ("ci-linux-x86_64.yml", "x86_64", "linux"),
        ("ci-sbsa.yml", "aarch64", "linux-aarch64"),
    ):
        jobs = yaml.safe_load((workflows / filename).read_text())["jobs"]
        assert jobs["generate-matrix"]["with"]["os"] == os_name
        callers = [
            job
            for job in jobs.values()
            if job.get("uses") == "./.github/workflows/_test-linux.yml"
            and not job["with"].get("python-only")
            and not job["with"].get("use-rtx")
        ]
        assert callers
        assert all(job["with"].get("architecture", "x86_64") == arch for job in callers)
        assert "executorch-runtime-build" not in jobs
    steps = _runtime_build_steps()
    build_at = next(
        i for i, step in enumerate(steps) if step.get("id") == "executorch-runtime"
    )
    assert any(
        step.get("name") == "Build the wheel (setup-py)" for step in steps[:build_at]
    )
...
def test_runtime_extension_has_dependency_wheel_rpaths():
    ...
    # No torch/lib or CUDA 12 path: this delegate links only the CUDA 13 runtime.
    assert entries == {
        "$ORIGIN",
...
def test_the_guard_actually_rejects_a_bad_artifact(tmp_path, case, expect_pass):
    ...
    # The toolkit can differ from torch.version.cuda, so check the linked major too.
...
@pytest.mark.unit
def test_runtime_wheel_version_is_independent_of_the_main_wheel(monkeypatch, tmp_path):
    """The companion uses its own base version, including for source-only builds."""
    function = _function_def(_runtime_setup_tree(), "get_runtime_version")
    ...
    exec(
        compile(
            ast.Module(body=[function], type_ignores=[]), str(_RUNTIME_SETUP_PY), "exec"
        ),
        namespace,
    )
    monkeypatch.setenv(
    ...
    assert namespace["get_runtime_version"]() == "0.1.0.dev20200103+cu132"
    monkeypatch.delenv("TORCH_TENSORRT_EXECUTORCH_RUNTIME_VERSION")
    monkeypatch.setattr(subprocess, "check_output", lambda *args, **kwargs: "abcdef0\n")
    assert namespace["get_runtime_version"]() == "0.1.0.dev0+abcdef0"
...
def test_cmake_version_uses_the_companion_distribution(tmp_path, requested):
    ...
    build = next(
        node
        for node in _runtime_setup_tree().body
    ...
    exec(
        compile(
            ast.Module(body=[build], type_ignores=[]), str(_RUNTIME_SETUP_PY), "exec"
        ),
        namespace,
    )
...
def test_runtime_readme_build_recipe_sets_the_version():
    """The optional version override in the build recipe must name the companion, not its dependency."""
    ...
    for recipe in recipes:
        ...
        for line in exports:
            ...
            assert (
                ...
                == (_RUNTIME_SETUP_PY.parent / "version.txt").read_text().strip()
            )
...
@pytest.mark.unit
@pytest.mark.parametrize("cuda", [None, "12.6", "12.8", "13.0", "13.2", "14.0"])
def test_runtime_wheel_pins_its_cuda_13_dependencies(monkeypatch, cuda):
    """Run setup with simulated installed versions and inspect the metadata it hands setuptools."""
    import runpy

    import setuptools

    fake_torch = types.ModuleType("torch")
    fake_torch.version = types.SimpleNamespace(cuda=cuda)
    fake_torch.__version__ = "2.15.0.dev20200103+cu130"
    ...
    def installed_version(name):
        if name in versions or name in {"tensorrt-cu12", "nvidia-cuda-runtime-cu12"}:
            queries.append(name)
    ...
    if cuda not in {"13.0", "13.2"}:
        with pytest.raises(RuntimeError, match="CUDA 13 is required"):
...
def test_the_wheel_checker_rejects_a_bad_wheel(tmp_path, case, should_pass):
    ...
    The real checker runs against generated wheels, and its shared-workflow invocation is checked
    for ordering, so changes to either side cannot silently remove validation.
    """
    from wheel.wheelfile import WheelFile
    ...
    check_at = next(
        ...
        if "check-executorch-runtime-wheel.py" in step.get("run", "")
    )
    check = steps[check_at]
    assert check["if"] == "${{ steps.executorch-runtime.outcome == 'success' }}"
    assert any("Repair Manylinux" in step.get("name", "") for step in steps[:check_at])
    assert any(
        step.get("name") == "Upload wheel to GitHub" for step in steps[check_at + 1 :]
    )
    ...
    elif case == "requires_a_conditional_main":
        requires = [
            r + '; python_version < "3.10"' if r.startswith("torch-tensorrt==") else r
            for r in requires
        ]
    ...
    completed = subprocess.run(
        [sys.executable, str(checker), str(wheel), "--architecture", arch],
        cwd=_REPO_ROOT,
...
def test_the_install_script_puts_a_cuda_runtime_on_the_library_path_for_both_majors():
    ...
    The generic install script also serves ordinary release and JetPack builds, which still
    need CUDA 12 support even though the slim delegate requires CUDA 13.
    """
    script = (_REPO_ROOT / ".github/scripts/install-torch-tensorrt.sh").read_text(
        encoding="utf-8"
    )
