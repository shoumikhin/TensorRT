...
def _has_required_headers(et_dir):
    return (
        et_dir.get_child("runtime/backend/interface.h").exists
        or et_dir.get_child(
...
def _local_executorch_impl(ctx):
    ...
    if not et_dir_str:
        fail(
            "EXECUTORCH_ROOT/EXECUTORCH_SOURCE_DIR is empty. Set one of them " + "or ensure executorch is installed in the active Python.",
        )
    ...
    if not _has_required_headers(et_dir):
        fail(
            "executorch at '"
            + et_dir_str
            + "' is missing runtime/backend/interface.h, "
